...
- (BOOL) renderWithCGLContext:(CGLContextObj)cgl_ctx forBounds:(NSRect)bounds
{
	...
	glTextureRangeAPPLE(GL_TEXTURE_2D, CVPixelBufferGetDataSize(_buffer), baseAddress);
    ...
    GLfloat originX = texRect.origin.x / (GLfloat)(_width + extraRight);
    GLfloat originY = texRect.origin.y / (GLfloat)(_height + extraTop);
    GLfloat extentX = originX + (texRect.size.width / (GLfloat)(_width + extraRight));
    GLfloat extentY = originY + (texRect.size.height / (GLfloat)(_height + extraTop));
    GLfloat texCoords[] = 
    {
        originX,    originY,
        extentX,	originY,
    ...
    /* GL_TEXTURE_RECTANGLE_ARB version
    GLfloat texCoords[] = 
    {
